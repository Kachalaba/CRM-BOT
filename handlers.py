...
def register_handlers(dispatcher: Dispatcher, bot_instance: Bot, admin_ids: list[int]):
    global dp, bot, ADMIN_IDS
    dp = dispatcher
    bot = bot_instance
    ADMIN_IDS = admin_ids
    ...
    async def send_welcome(message: types.Message):
        ...
        await message.answer(
            "Вітаю в CRM боті 🐬", reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard)
        )
            ...
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="✅ Підтвердити", callback_data=f"approve_deduction:{callback.from_user.id}"
                    ),
        ...
        for idx, row in enumerate(records):
            if str(row["ID"]) == user_id:
                ...
                sheets.history_sheet.append_row(
                    [user_id, datetime.now().strftime("%Y-%m-%d %H:%M"), "Списано 1 заняття"]
                )
    ...
    async def request_subscription(callback: CallbackQuery):
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="✅ Дати 10 занять", callback_data=f"approve_subscription:{callback.from_user.id}"
                    ),
