...
async def my_sessions(callback: CallbackQuery):
    ...
    for row in records:
        if str(row["ID"]) == user_id:
            ...
            await callback.message.answer(f"У вас залишилось {row['К-сть тренувань']} занять. Термін дії: {row['Кінцева дата']}")
            return
...
async def admin_panel(callback: CallbackQuery):
    ...
    for client in clients:
        ...
        await callback.message.answer(f"👤 {name} (ID: {user_id})\nЗалишок: {sessions}", reply_markup=keyboard)
